...
r_beers = requests.get("http://api.brewerydb.com/v2/locations?key=API_KEY_HERE&p=").json()
...
for current_page in range(1, num_pages + 1): 
    r_beers = requests.get("http://api.brewerydb.com/v2/locations?key=API_KEY_HERE&p=" + str(current_page)).json()
